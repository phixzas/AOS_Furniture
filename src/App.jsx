    ...
    <div className="site admin-mode">
      ...
      <main className="admin-panel">
          ...
          <div className="upload-grid">
            ...
            <section className="upload-card pricing-card"><div><p className="eyebrow">Shop settings</p><h2>Products &amp; prices</h2><p className="muted">Upload as many product photos as you need. Add a price to show it on the home page shop section.</p></div><input ref={galleryInput} type="file" accept="image/*" multiple onChange={(event) => { readFiles(event.target.files, 'gallery'); event.target.value = '' }} hidden /><button className="upload-button" type="button" onClick={() => galleryInput.current?.click()}>Upload product photos <span>+</span></button><div className="price-editor-grid">{gallery.slice(1).map((image) => <div className="price-editor" key={`price-${image.id}`}><img src={image.src} alt={image.alt} /><span>{image.alt}</span><input aria-label={`Price for ${image.alt}`} value={image.price || ''} onChange={(event) => setGallery((current) => current.map((item) => item.id === image.id ? { ...item, price: event.target.value } : item))} placeholder="e.g. ₦450,000" /><button className="delete-item-button" type="button" onClick={() => setGallery((current) => current.filter((item) => item.id !== image.id))}>Delete product</button></div>)}</div></section>
