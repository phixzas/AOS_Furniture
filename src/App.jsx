...
function PublicSite({ logo, gallery, products, services }) {
  ...
  const [shopMotionPaused, setShopMotionPaused] = useState(false)
  const [shopDirection, setShopDirection] = useState('forward')
  ...
  return (
    <div className="site">
      ...
      <main id="top">
          ...
          <section className="shop-section" id="shop"><div className="section-heading"><div><p className="eyebrow">Shop the collection</p><h2>Pieces to <em>keep.</em></h2></div><div><p className="shop-note">Every piece is available by request.<br />Message us on WhatsApp to order.</p><div className="shop-motion-controls"><button className="shop-motion-toggle" type="button" onClick={() => { setShopDirection('backward'); setShopMotionPaused(false) }}>Previous picture <span>↢</span></button><button className="shop-motion-toggle" type="button" onClick={() => setShopMotionPaused((current) => !current)}>{shopMotionPaused ? 'Resume movement' : 'Pause movement'} <span>{shopMotionPaused ? '↗' : 'Ⅱ'}</span></button><button className="shop-motion-toggle" type="button" onClick={() => { setShopDirection('forward'); setShopMotionPaused(false) }}>Next picture <span>↣</span></button></div></div></div><div className={`shop-track${shopMotionPaused ? ' shop-track-paused' : ''}${shopDirection === 'backward' ? ' shop-track-reverse' : ''}`} onTouchStart={() => setShopMotionPaused(true)} onPointerEnter={() => setShopMotionPaused(true)}>{[...products, ...products].map((image, index) => <article className="shop-card" key={`shop-${image.id}-${index}`}><div className="shop-image"><img src={image.src} alt={image.alt} />{image.price && <span className="price-tag">{image.price}</span>}</div><div className="shop-card-info"><span>{image.alt}</span><a className="order-button" href={`${whatsappLink}?text=${encodeURIComponent(`Hello AOS Furniture, I would like to order the ${image.alt}.`)}`} target="_blank" rel="noreferrer">Order Now <span>↗</span></a></div></article>)}</div></section>
