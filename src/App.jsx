    ...
    <div className="site admin-mode">
      ...
      <main className="admin-panel">
          ...
          <div className="upload-grid">
            ...
            <section className="upload-card services-editor"><div><p className="eyebrow">Services page</p><h2>Service names</h2><p className="muted">Edit existing services or add more for the Services page. Each one includes a Book now button to WhatsApp.</p></div><div className="service-name-grid">{services.map((service) => <label className="service-name-field" key={service.id}><span>{service.eyebrow}</span><input value={service.name} onChange={(event) => setServices((current) => current.map((item) => item.id === service.id ? { ...item, name: event.target.value } : item))} /></label>)}</div><button className="upload-button service-add-button" type="button" onClick={() => setServices((current) => [...current, { id: createServiceId(), name: 'New service', eyebrow: `${String(current.length + 1).padStart(2, '0')} / New service`, copy: '' }])}>Add another service <span>+</span></button></section>
          </div>
      </main>
