    ...
    <div className="site admin-mode">
      ...
      <main className="admin-panel">
          ...
          <div className="upload-grid">
            ...
            <section className="upload-card pricing-card"><div><p className="eyebrow">Shop settings</p><h2>Products &amp; prices</h2><p className="muted">Upload shop products separately. Add a price to show each one on the home page shop section.</p></div><input ref={galleryInput} type="file" accept="image/*" multiple onChange={(event) => { readFiles(event.target.files, 'product'); event.target.value = '' }} hidden /><button className="upload-button" type="button" onClick={() => galleryInput.current?.click()}>Upload product photos <span>+</span></button><div className="price-editor-grid">{products.map((image) => <div className="price-editor" key={`price-${image.id}`}><img src={image.src} alt={image.alt} /><span>{image.alt}</span><input aria-label={`Price for ${image.alt}`} value={image.price || ''} onChange={(event) => setProducts((current) => current.map((item) => item.id === image.id ? { ...item, price: event.target.value } : item))} placeholder="e.g. ₦450,000" /><button className="delete-item-button" type="button" onClick={() => setProducts((current) => current.filter((item) => item.id !== image.id))}>Delete product</button></div>)}</div></section>
  ...
  return (
    <div className="site">
      ...
      <main id="top">
          ...
          <section className="shop-section" id="shop"><div className="section-heading"><div><p className="eyebrow">Shop the collection</p><h2>Pieces to <em>keep.</em></h2></div><p className="shop-note">Every piece is available by request.<br />Message us on WhatsApp to order.</p></div><div className="shop-track">{[...products, ...products].map((image, index) => <article className="shop-card" key={`shop-${image.id}-${index}`}><div className="shop-image"><img src={image.src} alt={image.alt} />{image.price && <span className="price-tag">{image.price}</span>}</div><div className="shop-card-info"><span>{image.alt}</span><a href={`${whatsappLink}?text=${encodeURIComponent(`Hello AOS Furniture, I would like to order the ${image.alt}.`)}`} target="_blank" rel="noreferrer">Ask about this piece ↗</a></div></article>)}</div></section>
